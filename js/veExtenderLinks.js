...
function createDialogue(dialogueName, dialogueMessage, askQuery, template, templateResult) {
    ...
    dialogue.prototype.initialize = function () {
        ...
        dialogueInstance.dialogMode = 0; //0 = existing 1 = new
        ...
        switch (template) {
            case "File":
                ...
                var testDialogMode = function()
                {
                    ...
                    if(dialogueInstance.dialogMode == 0) {
                        ...
                        if (!dialogueInstance.isExistingFile && fileField.getValue() != null) { //nullcheck for getValue() may be required
            ...
            case "External link": {
                ...
                var testDialogMode = function()
                {
                    ...
                    if(dialogueInstance.dialogMode == 0) {
                        if (!dialogueInstance.isExistingFile && linkField.value.length != 0) {
                            if(dialogueInstance.suggestedFile != null) {
                                if (dialogueInstance.suggestedFile.hyperlink == linkField.value) {
                                    clearInputFields(fieldset);
                                    validator.cleanUpForm();
    ...
    if(exclude != null) {
        for (var i = 0; i < fieldset.getItems().length; i++) {
            ...
            for (var x = 0; x < exclude.length; x++)
                if (i == exclude[x])
                    ex = true;
            if(!ex) (fieldset.getItems()[i]).getField().setValue("");
        }
    }
    else
        for (var i = 0; i < fieldset.getItems().length; i++)
